...
设 $$dp[i]$$ 为前 i 天（0..i）能获得的最大利润，$$p$$ 是 `prices`。枚举**最后一次买入日**
$$j$$：在 j 天买、i 天卖，之前 j 天的收益是 $$dp[j]$$（同一天可以先卖后买，所以 j 既能是
上一次的卖出日，也能是这一次的买入日）：

$$
dp[i] = \max_{j<i}
\begin{cases}
dp[j] + p[i] - p[j], & p[i] > p[j] \\
dp[j], & \text{否则}
\end{cases}
$$

直接照这个式子写就是两层循环，$$O(n^2)$$：
...
class Solution {
    public int maxProfit(int[] prices) {
      ...
      for(int i = 1; i < n; i++) {
        for(int j = 0; j < i; j++) {
          dp[i] = Math.max(dp[i], dp[j] + Math.max(0, prices[i] - prices[j]));
        }
      }
      return dp[n-1];
    }
...
**第一步：去掉分支。** 当 $$p[i] \le p[j]$$ 时，$$dp[j] + p[i] - p[j] \le dp[j]$$，取
$$\max$$ 必然出局。所以可以无条件把两项都放进去，分支就消失了：

$$
dp[i] = \max\Bigl(\max_{j<i} dp[j],\;\; \max_{j<i}\bigl(dp[j] + p[i] - p[j]\bigr)\Bigr)
$$

**第二步：提常量。** $$p[i]$$ 与 $$j$$ 无关，提到 $$\max$$ 外面：

$$
\max_{j<i}\bigl(dp[j] + p[i] - p[j]\bigr) = p[i] + \max_{j<i}\bigl(dp[j] - p[j]\bigr)
$$

**第三步：最终式。** $$dp$$ 单调不减（多一天至少可以什么都不做），故
$$\max_{j<i} dp[j] = dp[i-1]$$：

$$
\boxed{\;dp[i] = \max\Bigl(dp[i-1],\;\; p[i] + \max_{j<i}\bigl(dp[j] - p[j]\bigr)\Bigr)\;}
$$

**内层循环为什么没了？** 令 $$B_i = \max_{j<i}\bigl(dp[j] - p[j]\bigr)$$，它里面不含
$$i$$，是一个**纯前缀最值**，满足

$$
B_i = \max\bigl(B_{i-1},\;\, dp[i-1] - p[i-1]\bigr)
$$

即 i 前进一步只需要补进一个新元素，$$O(1)$$ 更新。而原式
$$\max_{j<i}\bigl(dp[j] + p[i] - p[j]\bigr)$$ 含 $$p[i]$$，i 一变整组候选值全变，上一轮
算过的结果没法复用，只能重扫一遍——差别就在这里。
...
class Solution {
    public int maxProfit(int[] prices) {
      ...
      int[] dp = new int[n];
      dp[0] = 0;
      int best = dp[0] - prices[0];        // B_1 = max_{j<1}(dp[j]-p[j])
      for(int i = 1; i < n; i++) {
        dp[i] = Math.max(dp[i-1], prices[i] + best);
        best = Math.max(best, dp[i] - prices[i]);   // 补进 j=i，得到 B_{i+1}
      }
      return dp[n-1];
    }
}
...
- 这个 `best` 其实就是「持有股票状态」的另一种写法：`dp[i]` 是第 i 天手上没股票的最大
  收益，`best` 是手上有一股时的最大收益（已经把买入价扣掉了）。所以化简出来的式子和
  常见的 `hold / sold` 两状态 DP 是同一个东西。
- 121 是同一个套路的特例：那里只能买卖一次，$$dp[j]$$ 恒为 0，于是
  $$\max_{j<i}\bigl(dp[j] - p[j]\bigr) = -\min_{j<i} p[j]$$，前缀最值退化成了
  「前缀最小价格」，也就是上面 121 里的 `minPrice`。
- 套路本身可以脱离这道题记：**$$\max$$ 里带分支的先想想分支是否恒被淘汰 → 把与枚举
  变量 $$j$$ 无关的项提到 $$\max$$ 外 → 剩下的若是纯前缀最值就能 $$O(1)$$ 递推**，
  $$O(n^2)$$ 就降成 $$O(n)$$。反过来说，如果提完之后括号里还留着 $$i$$，那就说明候选值
  随 $$i$$ 整体变化，这条路走不通，得换单调队列 / 斜率优化之类的工具。

> **In English**: the naive DP for LC 122 enumerates the last buy day $$j$$, giving an
> $$O(n^2)$$ recurrence. Three rewrites collapse it to $$O(n)$$: (1) the
> $$p[i] \le p[j]$$ branch never wins the $$\max$$, so drop it; (2) $$p[i]$$ is
> independent of $$j$$, so pull it out of the $$\max$$; (3) what remains,
> $$\max_{j<i}\bigl(dp[j] - p[j]\bigr)$$, contains no $$i$$ — it is a pure prefix maximum
> and can be maintained in $$O(1)$$ per step. The inner loop disappears precisely because
> the surviving expression no longer depends on $$i$$, so past work is reusable. That
