---
layout: post
title: "AI Reading List"
date: 2025-02-10 11:45:00 +0800
tags: [ai, reading]
excerpt: "Keeping track of the AI papers, blogs, and videos I'm currently digesting."
---

What's on deck right now:

- The Scaling Laws trilogy, mainly for the compute vs. data trade-offs.
- The OpenAI Cookbook for hands-on recipes I can adapt quickly.
- Andrej Karpathy's *Neural Networks: Zero to Hero* video series.

I'll jot down takeaways for each item so I don't procrastinate on the summaries.
